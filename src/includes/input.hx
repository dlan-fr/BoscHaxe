...
class Input
{
	...
	public static function input(key:KeyPoll, gfx:Graphicsclass, control:Controlclass,updategraphicsmode:Controlclass->Void):Void {
		
	
		var i:Int, j:Int, k:Int;
		
		generickeypoll(control,key,updategraphicsmode);
		...
		if (control.list.active){
			...
			if (control.mx > 20 && control.mx < gfx.screenwidth - 12) {
				if (control.my > gfx.pianorollposition + gfx.linesize && control.my < gfx.pianorollposition + (gfx.linesize * 13)) {
					...
					control.cursory = Std.int((control.cursory - (control.cursory % gfx.linesize)) / gfx.linesize);
				...
				if (control.my > gfx.pianorollposition + gfx.linesize && control.my < gfx.pianorollposition + (gfx.linesize * 13)) {
					...
					control.notey = Std.int((control.notey - (control.notey % gfx.linesize)) / gfx.linesize);
				}
			}
			...
			if (key.click) {
				if (control.list.active){
					if (control.list.selection > -1) {
						...
						if (control.list.type == Controlclass.LIST_CATEGORY) {
						  control.list.close();
						...
						if (control.list.type == Controlclass.LIST_INSTRUMENT) {
							if (control.list.item[control.list.selection] == ">> Next Page") {
								control.voicelist.pagenum++;
								control.list.close();
								control.filllist(Controlclass.LIST_INSTRUMENT);
						...
						if (control.list.type == Controlclass.LIST_KEY) {
							control.changekey(control.list.selection);
							control.list.close();
						}
						if (control.list.type == Controlclass.LIST_SCALE) {
							control.changescale(control.list.selection);
							control.list.close();
						}
						
						if (control.list.type == Controlclass.LIST_BUFFERSIZE) {
							control.setbuffersize(control.list.selection);
							control.list.close();
						}
						
						if (control.list.type == Controlclass.LIST_EFFECTS) {
							control.effecttype = control.list.selection;
							control.updateeffects();
							control.list.close();
						}
						
						if (control.list.type == Controlclass.LIST_SCREENSIZE) {
					...
					if (control.mx < (gfx.screenwidth-40) / 4) {
						...
						control.filllist(Controlclass.LIST_SCREENSIZE);
					...
					if (control.currenttab == 0) {
						...
						if (control.instrumentcury > -1) {
							...
							if (control.my > (gfx.linesize * 2) + 3 && control.my < (gfx.linesize * 3) + 3) {
								if (control.mx > 140 && control.mx < 230) {
									control.filllist(Controlclass.LIST_CATEGORY);
						...
						if (Help.inboxw(control.mx, control.my, 120, (gfx.linesize * 3), 70, 10)) {
							control.filllist(Controlclass.LIST_BUFFERSIZE);
						...
						if (Help.inboxw(control.mx, control.my, gfx.screenwidth - 150, (gfx.linesize * 3), 25, 10)) {
							control.filllist(Controlclass.LIST_EFFECTS);
					...
					if (control.currentbox > -1) {
						if (!control.list.active) {
							if (control.mx < 150) {
								control.filllist(Controlclass.LIST_SELECTINSTRUMENT);
				...
				if (control.currenttab == 3) {
					...
					if (control.my > gfx.linesize && control.my < gfx.pianorollposition + 10) {				
						if (control.mx >= 140 && control.my > 35 && control.mx < gfx.screenwidth - 25) {
							...
							if (k == 1) {
								control.musicbox[control.currentbox].cutoffgraph[control.looptime%control.boxcount] = Std.int((i * 128) / 200);
								control.musicbox[control.currentbox].resonancegraph[control.looptime%control.boxcount] = Std.int((j * 9) / 45);
								...
								control.instrument[control.currentinstrument].setfilter(Std.int((i * 128) / 200), Std.int((j * 9) / 45));
							...
							if (k == 1) {
								control.musicbox[control.currentbox].volumegraph[control.looptime%control.boxcount] = Std.int( (j * 256) / 45);
								control.instrument[control.currentinstrument].changevolumeto(Std.int((j * 256) / 45));
								if (control.instrument[control.currentinstrument].type > 0) {
									control.drumkit[control.instrument[control.currentinstrument].type-1].updatevolume(Std.int((j * 256) / 45));
								}
							}else{
								control.instrument[control.currentinstrument].setvolume(Std.int((j * 256) / 45));
								control.instrument[control.currentinstrument].updatefilter();
								if (control.instrument[control.currentinstrument].type > 0) {
									control.drumkit[control.instrument[control.currentinstrument].type-1].updatevolume(Std.int((j * 256) / 45));
								}
	...
	public static function generickeypoll(control:Controlclass,key:KeyPoll,updategraphicsmode:Controlclass->Void):Void {
